...
get_srss_st <- function(etuff){
  ...
  for (i in 1:nrow(srss)){
    ...
    sr <- suntools::sunriset(cbind(srss$longitude[i], srss$latitude[i]), lubridate::with_tz(srss$DateTime[i], srss$tz[i]), direction = "sunrise", POSIXct.out = TRUE)$time
    if (as.Date(substr(sr, 1, 10)) != as.Date(srss$DateTime[i])){
      ...
      sr <- suntools::sunriset(cbind(srss$longitude[i], srss$latitude[i]), srss$DateTime[i], direction = "sunrise", POSIXct.out = TRUE)$time
    ...
    ss <- suntools::sunriset(cbind(srss$longitude[i], srss$latitude[i]), lubridate::with_tz(srss$DateTime[i], srss$tz[i]), direction = "sunset", POSIXct.out = TRUE)$time
    if (as.Date(substr(ss, 1, 10)) != as.Date(srss$DateTime[i])){
      ...
      ss <- suntools::sunriset(cbind(srss$longitude[i], srss$latitude[i]), srss$DateTime[i], direction = "sunset", POSIXct.out = TRUE)$time
